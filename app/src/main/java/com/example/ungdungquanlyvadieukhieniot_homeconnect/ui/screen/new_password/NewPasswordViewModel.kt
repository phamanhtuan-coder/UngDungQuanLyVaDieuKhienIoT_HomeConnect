...
sealed class NewPassWordState {
    ...
    data class Success(val success: Boolean, val exists: Boolean?, val message: String) : NewPassWordState()
...
//                _newPasswordState.value = NewPassWordState.Success(response.success, true, response.message)
                ...
                _newPasswordState.value = NewPassWordState.Error(false, e.message ?: "Tạo mật khẩu mơí thất bại!")
            }
        }
    }
}